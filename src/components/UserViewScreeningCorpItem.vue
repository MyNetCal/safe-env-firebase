...
async function toggleRequiringStatusReasons(reason = '', newDate) {
  ...
  userCorps.forEach(async (uc) => {
    ...
    if (backgroundCheckExpired) {
      ...
      // Only downgrade a user who was already approved. A user still going
      // through approval must stay 'Pending Approval' so the committee can keep
      // voting (otherwise the vote buttons hide).
